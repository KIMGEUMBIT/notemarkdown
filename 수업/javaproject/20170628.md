...
if (e.getSource() == btnInsert) {// 신규버튼을 눌렀다면
  if (isInsert == false) { // Insert(신규)버튼을 누르지 않은 상태에서 처음 누른 경우
    txtCirum.setEditable(true);// 편집 할 수 있는 //setEnable()// 버튼 누를 수 있나 없나 활성화
...
```java
    ...
    private void init() {
        try {
            ...
            rs.last(); // 마지막 레코드로 이동하여 핸재행으로 인식한 후 전체 값을 구하는것 근데 count로 해도 괜춘한것 같은데
...
```java
  ...
  //yes|no? -> JOptionPane.showConfirmDialog(1.부모창, 2.메세지 내용)
```
...
```java
[김금빛] [오전 11:08] 자바 대화상자창

JOptionPane.showMessageDialog(this, "해당자료는 없습니다");
//사용자로부터 값을 입력->JOptionPane.showInputDialog
//간단한 경고->JOptionPane.showMessageDialog
//yes|no? -> JOptionPane.showConfirmDialog(1.부모창, 2.메세지 내용)
[김금빛] [오전 11:17] import javax.swing.*;
[김금빛] [오전 11:48]             txtCbun.setText(rs1.getString("c_bun"));
[김금빛] [오전 11:52] if (e.getSource() == btnCbun) {// 고객버튼을 눌렀다면
...
[김금빛] [오후 12:11]          rs2 = stmt.executeQuery(sql);
[김금빛] [오후 12:12]             btnBbun.setText(rs2.getString("b_bun"));
[김금빛] [오후 12:17]  else if (e.getSource() == btnDaeyeo) {
         //1.대여 도서 제목을 메모에 표시(고객정보)
         if(taCmemo.getText().equals("")) { //처음 빌린 경우
            taCmemo.setText(txtBjemok.getText());
         }   else { //한권이상 빌린 경우 (도서제목, 도서제목2, 도서제목3)
         ...
         try {
            ...
            rs1.next(); //찾은 레코드로 이동하여라 if(rs1.next()==true)

         }catch(Exception e2) {
            System.out.println("고객대여과정에서 오류발생"+e2);
         }
      }
[김금빛] [오후 12:36]          try {
            //1. 고객의 대여수를 증가
...
#### stmt.executeQure(), stmt.executeUpdate()
...
```java
     ...
     try {
       ...
       int update = stmt.executeUpdate(sql); //성공-> 변경 행 수, 실패-> 0
       System.out.println("고객정보수정 성공유무(update)=>"+update);
       bg=false; // 다시 불활성화
       rs1.close(); //finally에서 사용해야한다
     }catch(Exception e2) {
       System.out.println("고객대여과정에서 오류발생"+e2);
       return;
     }
